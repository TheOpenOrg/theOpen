...
function newClientAuto() {
    CLIENT="$1"
    PASS="$2"  # 1 = без пароля, 2 = с паролем
    ...
    if [[ ! $PASS =~ ^[1-2]$ ]]; then
        echo "Invalid password option: $PASS"
        exit 1
    fi

    CLIENTEXISTS=$(tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep -c -E "/CN=$CLIENT\$")
    if [[ $CLIENTEXISTS == '1' ]]; then
        echo "The specified client CN already exists, please choose another name."
        exit 1
    fi
    ...
    case $PASS in
        1)
            ...
            ;;
        2)
            echo "⚠️ Password input will be required ⚠️"
            EASYRSA_CERT_EXPIRE=3650 ./easyrsa --batch build-client-full "$CLIENT"
            ;;
    esac
    ...
    echo "Config file generated at $homeDir/$CLIENT.ovpn"
}

# Check for root, TUN, OS...
initialCheck

if [[ "$1" == "--new-client" && -n "$2" ]]; then
    CLIENT_NAME="$2"
    PASS_MODE="${3:-1}"  # Default: 1 = no password
    newClientAuto "$CLIENT_NAME" "$PASS_MODE"
    exit 0
fi
